...
import Customer from "../CreateNewOrder/Customer";
...
const DineDashboard = () => {
  ...
  const fetchData = (key) => {
    ...
    if (date !== "lastweek") {
      const res = axios.get(
        `${BASE_URL}/reports/graph?order_type=1&date=${date}`,
        {
          headers: {
  ...
  const dateRangeData = async (start, end) => {
    ...
    await axios
      .get(
        `${BASE_URL}/reports/graph/range?from=${startDate}&to=${endDate}&order_type=1`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `bearer ${token}`,
  ...
  const statsData = () => {
    const res = axios.get(
      `${BASE_URL}/reports/stats/dashboard/top?order_type=1`,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `bearer ${token}`,
